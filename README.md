...
Shevtsova NA, Lockhart AB, Rybak IA, Magnuson DSK, Danner SM. **Linking spinal circuit reorganization to recovery after thoracic spinal cord injury.** *eLife*. 2026;14:RP107480. doi: [10.7554/eLife.107480.3](https://doi.org/10.7554/eLife.107480.3)

_Preprint version (earlier title):_ Shevtosva NA, Lockhart AB, Rybak IA, Magnuson DSK, Danner SM. **Reorganization of spinal neural connectivity following recovery after thoracic spinal cord injury: insights from computational modelling.** *bioRxiv*, 2025.05.17.654682; doi: [10.1101/2025.05.17.654682](https://doi.org/10.1101/2025.05.17.654682)
...
```
Note, [CPGNetworkSimulator](https://github.com/SimonDanner/CPGNetworkSimulator) requires Boost headers. Ensure Boost is installed and in your include path.
...
Use `bifurcation_diagrams.ipynb`. [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)][colab-link_bifurcations].

### Data analysis and figures
Use `analyze_results.ipynb`. [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)][colab-link_analysis]
...
- `analyze_sensitivity.ipynb`  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)][colab-link_sensitivity]: loads simulation results, fits surrogate model, performs Sobol analysis.
...
- `sensitivity/*.npz`: Saved simulation results from the parameter-perturbation runs (generated by `sensitivity_run.py` and loaded by `analyze_sensitivity.ipynb` for surrogate fitting and Sobol analysis).
...
- `analyze_sensitivity.ipynb`: surrogate modeling, Sobol sensitivity analysis, and respective plots. [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)][colab-link_sensitivity]
...
[colab-link_sensitivity]: https://colab.research.google.com/github/dannerlab/rat-sci-locomotion-model/blob/master/analyze_sensitivity.ipynb
